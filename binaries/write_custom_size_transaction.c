...
#include <string.h>
#include <errno.h>
#include <sys/ioctl.h>  // Required for ioctl() and _IO() macros

#define CHUNK_SIZE (1UL * 1024 * 1024)    // 1 MB per write

// Define custom ioctl transaction commands
...
int main(int argc, char *argv[]) {
    ...
    /* Perform the chunked writes within the transaction */
    while (total_written < file_size) {
        ...
        if (n < 0) {
            ...
            // Optional: You might want an ABORT_TRANSACTION ioctl here in a real scenario
            free(buffer);
            close(fd);
            return 1;
        }
