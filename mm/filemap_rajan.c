...
vm_fault_t filemap_fault(struct vm_fault *vmf)
{
	...
	int is_module = is_ext4_module_sb(sb);
	int is_child =0;
	if(is_module) is_child = scorw_is_child_file(inode, 1);
	//scorw end//
	...
	if (likely(!IS_ERR(folio))) {
		...
		if (!(vmf->flags & FAULT_FLAG_TRIED))
			if(is_child==0) fpin = do_async_mmap_readahead(vmf, folio); //scorw start end//
		...
		ret = VM_FAULT_MAJOR;
		if(is_child==0) fpin = do_sync_mmap_readahead(vmf); //scorw start end//
retry_find:
		/*
...
page_not_uptodate:
	...
	fpin = maybe_unlock_mmap_for_io(vmf, fpin);
	//scorw start//
	if (is_module && is_child) {
		//prepare the follow on read//
		struct kiocb kiocb;
		struct iov_iter to;
		struct iovec iov;
		ssize_t ret;
		...
		/* prepare synchronous single-page kiocb + iterator */
		init_sync_kiocb(&kiocb, file);
		kiocb.ki_pos = pos;

		iov.iov_base = kmap_local_folio(folio, 0);
		iov.iov_len = PAGE_SIZE;
		iov_iter_init(&to, WRITE, &iov, 1, PAGE_SIZE);

		/* custom scorw read path */
		ret = scorw_follow_on_read_child_blocks(inode, &kiocb, &to);
